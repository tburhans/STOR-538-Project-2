...
```{r}
FINAL_DATASET <- read.csv(url('https://raw.githubusercontent.com/tburhans/STOR-538-Project-2/main/FinalDataset.csv'))
```
...
```{r}
GAMES_DETAILS$MIN <- as.character(GAMES_DETAILS$MIN)
...
First, I subsetted the data to for the years 2014 and above.
...
DataSet_TOTAL_2020 <- Dataset_2020[,-c(1,2,3,4,5,6,7)]

df<- subset(FINAL_DATASET, GAME_YEAR>=2014) 
TestingDataSet <- df[,-c(1,2,3,4,5,6,7)]

```

```{r}
any(is.na(TestingDataSet))
```

To identify significant variables, I will use the glm function (used to fit generalized linear models). 

```{r}
fit <- glm(Total ~ OREB_home + OREB_away + OREB + DREB_home + DREB_away + DREB + REB_home + REB_away + REB + AST_home + AST_away + AST + STL_home + STL_away + STL + BLK_home + BLK_away + BLK + TO_home + TO_away + TO + FGA_home + FGA_away + FGA + FTA_home + FTA_away + FTA + OREB_Starters_home + OREB_Starters_away + Starters_OREB + DREB_Starters_home + DREB_Starters_away + Starters_DREB + REB_Starters_home + REB_Starters_away + Starters_REB + AST_Starters_home + AST_Starters_away + Starters_AST + STL_Starters_home + STL_Starters_away + Starters_STL + BLK_Starters_home + BLK_Starters_away + Starters_BLK + TO_Starters_home + TO_Starters_away + Starters_TO + OREB_Bench_home + OREB_Bench_away + Bench_OREB + DREB_Bench_home + DREB_Bench_away + Bench_DREB + REB_Bench_home + REB_Bench_away + Bench_REB + AST_Bench_home + AST_Bench_away + Bench_AST + STL_Bench_home + STL_Bench_away + Bench_STL + BLK_Bench_home + BLK_Bench_away + Bench_BLK + TO_Bench_home + TO_Bench_away + Bench_TO + eFG_Starters_home + eFG_Starters_away + Starters_eFG + Home_Possesions + Away_Possesions + Home_Points_Per_Possesion + Away_Points_Per_Possesion + Home_Turnover_Percentage + Away_Turnover_Percentage + Home_OREB_Percentage + Away_OREB_Percentage + Home_DREB_Percentage + Away_DREB_Percentage + Home_AST_TO_Ratio + Away_AST_TO_Ratio, data=TestingDataSet, family=poisson())

summary(fit)
```

After using the generalized linear model under the poisson family (poisson regression), I created a neural net model using the significant variables produced from glm.  
...
dfNorm <- as.data.frame(lapply(TestingDataSet, normalize))
```

```{r}
n = nrow(TestingDataSet)
...
```{r}
nn = neuralnet(Total ~ OREB_home + OREB_away + FGA_home + FGA_away + FTA_home + FTA_away + Home_Points_Per_Possesion + Away_Points_Per_Possesion, data=train, hidden = 8, act.fct = "logistic", linear.output = FALSE, stepmax=10^5, threshold=0.01)
